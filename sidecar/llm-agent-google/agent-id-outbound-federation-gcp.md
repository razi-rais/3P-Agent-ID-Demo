...
The deployment in this article spans four observability surfaces: the Entra **sign-in logs** for everything that touches `login.microsoftonline.com`, the ACA-attached **Log Analytics workspace** (LAW) for everything the sidecar and the protected API print to stdout, the GCP **Cloud Audit Logs** for everything that touches a GCP API, and the **BigQuery sink** for keeping any of those past their default retention. Each surface answers a different question about the same chat turn, and the union of the four is what an auditor or an on-call reviewer needs to reconstruct the federation chain end-to-end.

A few acronyms recur below and are worth pinning before the queries start. **LAW** is Log Analytics workspace, the Kusto-backed store that ACA writes container stdout to. **KQL** is Kusto Query Language, the language LAW queries are written in. **RFC 8693** is the OAuth 2.0 Token Exchange spec. The `grantType` and `subjectTokenType` values in the STS audit log entries are RFC 8693 URNs. **Cloud Audit Logs** is GCP's umbrella term for three log streams produced by every GCP service: *Admin Activity* (always on, free, 400-day retention), *Data Access* (off by default, billed per GiB, 30-day default retention), and *System Event* (always on, free). The token exchange and impersonation calls in this article are Data Access events and are therefore not captured until Data Access logging is explicitly enabled for the three services involved.

### 10.1 The observability map

Every step of the chain produces an audit event somewhere. The table below names each event, says which surface it lands on, and shows the query verb you use to pull it. Reading top to bottom, the table reconstructs the same flow walked in §7, this time from the audit-log perspective.

| # | Step | Event recorded | System | Query surface | Default retention |
|---|------|---|---|---|---|
| 1 | Agent SP authenticates to Entra to mint the GCP-leg token | Service-principal sign-in (with the `agent` block populated) | Entra ID | Graph API `auditLogs/signIns`, or KQL on `AADServicePrincipalSignInLogs` if exported to LAW | 30 days in Entra, longer if exported |
| 2 | Sidecar finishes the OBO / client-credential exchange | MSAL `=== Token Acquisition finished successfully` line | ACA container stdout | LAW `ContainerAppConsoleLogs_CL` (KQL) | Per LAW retention setting (default 30 days) |
| 3 | Agent POSTs the Entra JWT to GCP STS as a `subject_token` | `google.identity.sts.v1.SecurityTokenService.ExchangeToken` | GCP Cloud Audit Logs | `gcloud logging read serviceName="sts.googleapis.com"` | 30 days (Data Access) |
| 4 | Federated token is exchanged for a SA access token | `iamcredentials.googleapis.com:GenerateAccessToken` | GCP Cloud Audit Logs | `gcloud logging read serviceName="iamcredentials.googleapis.com"` | 30 days (Data Access) |
| 5 | Agent calls Vertex AI under the impersonated SA | `aiplatform.googleapis.com:GenerateContent` | GCP Cloud Audit Logs | `gcloud logging read serviceName="aiplatform.googleapis.com"` | 30 days (Data Access) |
| 6 | Agent SP authenticates to Entra again, this time for the protected API | Second service-principal sign-in, with `resourceDisplayName` of the protected API | Entra ID | Same as row 1 | 30 days |
| 7 | Protected API validates the JWT and authorizes the role | `[FULL VALIDATION]`, `[AUDIENCE CHECK]`, `[ROLE CHECK]`, `[WEATHER REQUEST]` lines | ACA container stdout | LAW `ContainerAppConsoleLogs_CL` (KQL) | Per LAW retention setting |

A note on retention before moving on. The Entra sign-in logs are retained for 30 days in the portal by default. Longer retention requires exporting them to a Log Analytics workspace or a third-party SIEM. GCP Cloud Audit Logs have separate retention per log type: Admin Activity is 400 days, Data Access is 30 days, System Event is 400 days. When the operator needs anything past 30 days for the STS / iamcredentials / aiplatform entries, a BigQuery (or Cloud Storage) sink has to be in place before the event occurs, not after. The sink command is in §10.5.

### 10.2 Entra side: service-principal sign-ins

Every time the sidecar mints a token for one of the configured DownstreamApis, the Agent's service principal authenticates against the v2 token endpoint, and that authentication shows up as a service-principal sign-in. The two sign-ins to look for on a chat turn are the GCP-leg mint (resource = Blueprint, audience = `api://<Blueprint>`) and the protected-API-leg mint (resource = the protected API). The Graph API verb is the same in both cases. Only the `resourceId` filter differs.

The OData filter for service-principal sign-ins requires the `signInEventTypes` predicate. Without it, the query returns interactive user sign-ins only and finds zero rows for an agent. The `appId` filter is on the Agent's app id, which equals the Agent's `oid` for a Microsoft Entra Agent ID.

```bash
# Pull the most recent two sign-ins for the Agent SP since a given time
az rest --method GET --url \
  "https://graph.microsoft.com/beta/auditLogs/signIns?\$filter=signInEventTypes/any(t:t eq 'servicePrincipal') and appId eq '<agent-app-id>' and createdDateTime ge 2026-06-04T05:55:00Z&\$top=2"
```

A single entry from a real run, scrubbed of tenant-specific identifiers, looks like this:

```json
{
  "id": "b20f26bf-668c-4eb1-b5e1-15df227c0200",
  "correlationId": "2e20bb93-1829-4ef6-b56f-f041fed6878e",
  "createdDateTime": "2026-06-04T05:59:39Z",
  "appId": "aaaaaaaa-1111-2222-3333-bbbbbbbbbbbb",
  "appDisplayName": "Weather Agent (GCP Prod)",
  "servicePrincipalId": "aaaaaaaa-1111-2222-3333-bbbbbbbbbbbb",
  "servicePrincipalName": "Weather Agent (GCP Prod)",
  "resourceId": "dddddddd-1111-2222-3333-eeeeeeeeeeee",
  "resourceDisplayName": "Weather API (GCP Prod)",
  "resourceServicePrincipalId": "eeeeeeee-1111-2222-3333-ffffffffffff",
  "signInEventTypes": ["servicePrincipal"],
  "clientCredentialType": "federatedIdentityCredential",
  "federatedCredentialId": "1cc24895-9b1f-4702-84ce-6abed68a3d69",
  "conditionalAccessAudiences": ["dddddddd-1111-2222-3333-eeeeeeeeeeee"],
  "agent": {
    "agentSubjectType": "notAgentic",
    "agentType": "agenticAppInstance",
    "parentAppId": "bbbbbbbb-1111-2222-3333-cccccccccccc"
  },
  "ipAddress": "20.22.x.x",
  "status": { "errorCode": 0 }
}
...
Two fields in this entry carry most of the audit value. The `agent` block names the sign-in as an `agenticAppInstance` and pins the `parentAppId` to the Blueprint's appId. This is what tells an auditor that the principal that just signed in is governed by Blueprint `bbbbbbbb-...`, without having to cross-reference against the directory. The `clientCredentialType: federatedIdentityCredential` confirms the sign-in succeeded against a federated identity credential (the Agent's FIC against the UAMI), not a stored client secret. The `federatedCredentialId` is the FIC's object id, which can be looked up against `/applications/<app>/federatedIdentityCredentials` to confirm which FIC was actually used.

A chat turn that hits a token cache in the sidecar produces no new sign-in entry, because no new authentication occurred. If the audit window is short and the entry is missing, restart the agent revision to clear the in-process cache before running the chat turn:

```bash
az containerapp revision restart -g rg-agent-gcp-prod -n agent-app-agentid \
  --revision $(az containerapp revision list -g rg-agent-gcp-prod \
    -n agent-app-agentid --query '[0].name' -o tsv)
```

Sign-in entries can take 5 to 15 minutes to propagate to the Graph API after the actual authentication. GCP audit log entries typically appear within 30 seconds. When the two sides are queried within the same minute, the GCP entries arrive first.

### 10.3 ACA side: Log Analytics workspace

ACA forwards every container's stdout to the LAW attached to the Container Apps environment. The relevant table is `ContainerAppConsoleLogs_CL`. The two containers worth filtering on are `sidecar` (the Entra SDK, where MSAL prints `=== Token Acquisition ...` lines on every mint) and `weather-api` (the protected API, where each request prints `[FULL VALIDATION]`, `[AUDIENCE CHECK]`, `[ROLE CHECK]`, and `[WEATHER REQUEST]` lines).

The LAW customer id is on the workspace resource and is the only thing the query verb needs:

```bash
LAW_ID=$(az monitor log-analytics workspace show -g rg-agent-gcp-prod \
  -n law-agent-gcp --query 'customerId' -o tsv)

az monitor log-analytics query --workspace $LAW_ID --analytics-query "
ContainerAppConsoleLogs_CL
| where TimeGenerated between (datetime(2026-06-04T05:59:00Z)
                            .. datetime(2026-06-04T06:00:30Z))
| where ContainerName_s in ('sidecar', 'weather-api')
| where Log_s has_any ('Token Acquisition', 'FULL VALIDATION',
                       'AUDIENCE CHECK', 'ROLE CHECK',
                       'WEATHER REQUEST')
...
The output for the same 90-second window as the Entra sign-in shown above (PII-scrubbed) reads:

```
TimeGenerated                | Container    | Log line
─────────────────────────────┼──────────────┼──────────────────────────────────
2026-06-04T05:59:34.890Z     | sidecar      | === Token Acquisition (ClientCredentialRequest) started
2026-06-04T05:59:37.942Z     | sidecar      | === Token Acquisition finished successfully
2026-06-04T05:59:38.915Z     | sidecar      | === Token Acquisition finished successfully
2026-06-04T05:59:39.906Z     | sidecar      | === Token Acquisition finished successfully
2026-06-04T05:59:39.999Z     | weather-api  | [FULL VALIDATION] Signature verified, issuer:
                             |              |   https://login.microsoftonline.com/<tenant>/v2.0,
                             |              |   exp: 1780556379
2026-06-04T05:59:39.999Z     | weather-api  | [AUDIENCE CHECK] aud=dddddddd-1111-2222-3333-eeeeeeeeeeee
2026-06-04T05:59:39.999Z     | weather-api  | [ROLE CHECK] roles=['Weather.Read.All']
2026-06-04T05:59:41.519Z     | weather-api  | [WEATHER REQUEST] City: London, Temp: 57°F
```

Three sidecar `Token Acquisition finished` lines for one chat turn is expected. The sidecar mints one token per configured DownstreamApi the agent calls. The agent calls the GCP STS DownstreamApi to get the JWT to send to GCP, then calls the protected-API DownstreamApi to get the JWT for the Weather API, and an extra Graph mint may occur on first hit per replica. The `weather-api` lines arrive in the same wall-clock second because they are emitted from inside one HTTP request handler.

### 10.4 GCP side: Cloud Audit Logs

GCP Cloud Audit Logs are the single most useful surface for the federation chain, because they are the only place that records what GCP saw on the wire: the exact `audience`, the exact `mapped_principal`, the exact `serviceAccountDelegationInfo` chain. The cost is that Data Access logs are off by default for every service. Without enabling them, the operator sees nothing from STS, nothing from iamcredentials, and nothing from Vertex AI on the model-inference path. The three services have to be enabled at the project level:

```bash
PROJECT=acme-agent-prod-001

# Enabling Data Access for STS, IAM (the umbrella for iamcredentials),
# and Vertex AI. Note: iamcredentials.googleapis.com is rejected as a
# service name here; iam.googleapis.com is the correct umbrella.
gcloud projects get-iam-policy $PROJECT --format=json > /tmp/policy.json
# Edit /tmp/policy.json's auditConfigs to include the three services
# with ADMIN_READ, DATA_READ, and DATA_WRITE, then:
gcloud projects set-iam-policy $PROJECT /tmp/policy.json
```

The three queries that produce the evidence for one chat turn follow. Each one points at a different `serviceName` and filters by timestamp. The `limit=1` is for showing one entry. Drop it to pull the full window.

#### 10.4.1 STS token exchange

```bash
gcloud logging read \
  'protoPayload.serviceName="sts.googleapis.com"
   AND timestamp>="2026-06-04T05:59:30Z"' \
  --project=$PROJECT --limit=1 --format=json
```

A real entry, scrubbed:

```json
{
  "timestamp": "2026-06-04T05:59:38.221Z",
  "logName": "projects/acme-agent-prod-001/logs/cloudaudit.googleapis.com%2Fdata_access",
  "protoPayload": {
    "@type": "type.googleapis.com/google.cloud.audit.AuditLog",
    "serviceName": "sts.googleapis.com",
    "methodName": "google.identity.sts.v1.SecurityTokenService.ExchangeToken",
    "authenticationInfo": {
      "principalSubject": "aaaaaaaa-1111-2222-3333-bbbbbbbbbbbb"
    },
    "metadata": {
      "@type": "type.googleapis.com/google.identity.sts.v1.AuditData",
      "mapped_principal": "principal://iam.googleapis.com/projects/100000000001/locations/global/workloadIdentityPools/entra-prod-pool/subject/aaaaaaaa-1111-2222-3333-bbbbbbbbbbbb"
    },
    "request": {
      "@type": "type.googleapis.com/google.identity.sts.v1.ExchangeTokenRequest",
      "audience": "//iam.googleapis.com/projects/100000000001/locations/global/workloadIdentityPools/entra-prod-pool/providers/entra-agentid-provider",
    ...
    "requestMetadata": {
      "callerIp": "20.22.x.x",
      "callerSuppliedUserAgent": "python-requests/2.34.2"
    },
    "resourceName": "projects/100000000001/locations/global/workloadIdentityPools/entra-prod-pool/providers/entra-agentid-provider",
    "status": {}
  }
}
```

This single entry encodes the entire WIF contract from GCP's perspective. The `authenticationInfo.principalSubject` is the Agent's `oid` taken verbatim from the JWT's `sub` claim, which is what the provider's CEL `attributeCondition` was pinned against in §5. The `metadata.mapped_principal` is the principal URI the provider's `attributeMapping` produced. This is the exact string the SA's IAM binding has to grant `roles/iam.workloadIdentityUser` to. The `request.audience` is what the provider's `allowedAudiences` was checked against. The three URN values under `request` confirm RFC 8693 token exchange. The empty `status` is GCP's convention for success. (An `authorizationInfo[]` entry with `granted: false` on `sts.identityProviders.checkLogging` may also appear. That is a separate ADMIN_READ permission check on whether the caller can read logging config, not the token-exchange result. The exchange itself succeeded because the entry exists in `data_access` with `methodName: ExchangeToken` and a populated `mapped_principal`.)

#### 10.4.2 iamcredentials impersonation
...
```json
{
  "timestamp": "2026-06-04T05:59:38.306Z",
  "protoPayload": {
    "serviceName": "iamcredentials.googleapis.com",
    "methodName": "GenerateAccessToken",
    "authenticationInfo": {
      "principalSubject": "principal://iam.googleapis.com/projects/100000000001/locations/global/workloadIdentityPools/entra-prod-pool/subject/aaaaaaaa-1111-2222-3333-bbbbbbbbbbbb"
    },
    "authorizationInfo": [
      {
        "granted": true,
    ...
    "metadata": {
      "identityDelegationChain": [
        "projects/-/serviceAccounts/weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com"
      ]
    },
    "request": {
      "name": "projects/-/serviceAccounts/weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com"
    },
    "requestMetadata": {
      "callerIp": "20.22.x.x",
      "callerSuppliedUserAgent": "python-requests/2.34.2,gzip(gfe)"
    },
  ...
  "resource": {
    ...
    "labels": {
      "email_id": "weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com",
      "project_id": "acme-agent-prod-001",
...
This entry is the second half of the federation handshake. The `authenticationInfo.principalSubject` is now the *WIF principal URI*, not the Agent's `oid` directly, but the `principal://...subject/<oid>` form that the provider mapped it to. GCP is saying: "the WIF principal corresponding to subject `aaaaaaaa-...` is asking to impersonate the service account named below." The `authorizationInfo[0].granted: true` for `iam.serviceAccounts.getAccessToken` is the proof that the SA's IAM policy includes `roles/iam.workloadIdentityUser` for that WIF principal. The `metadata.identityDelegationChain` is the chain that produced the final access token (single hop here). The `resource.labels.unique_id` is the SA's numeric id, useful for cross-referencing other SAs over time.

#### 10.4.3 Vertex AI inference
...
```json
{
  "timestamp": "2026-06-04T05:59:39.717Z",
  "protoPayload": {
    "serviceName": "aiplatform.googleapis.com",
    "methodName": "google.cloud.aiplatform.v1beta1.PredictionService.GenerateContent",
    "authenticationInfo": {
      "principalEmail": "weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com",
      "principalSubject": "serviceAccount:weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com",
    ...
    "authorizationInfo": [
      {
        ...
        "resource": "projects/acme-agent-prod-001/locations/us-central1/publishers/google/models/gemini-2.5-flash-lite"
      }
    ],
    "request": {
      "model": "projects/acme-agent-prod-001/locations/us-central1/publishers/google/models/gemini-2.5-flash-lite"
    },
    "requestMetadata": {
      "callerSuppliedUserAgent": "grpc-python/1.81.0 grpc-c/54.0.0 (linux; chttp2),gzip(gfe)"
    },
    "resourceName": "projects/acme-agent-prod-001/locations/us-central1/publishers/google/models/gemini-2.5-flash-lite"
  }
}
```

This is the audit entry most worth reading slowly. The `authenticationInfo.principalEmail` is the SA, which is the obvious caller. The single field that connects this Vertex AI call to the Entra Agent Identity is `authenticationInfo.serviceAccountDelegationInfo[0].principalSubject`, the WIF principal URI, which contains the Agent's `oid` as its `subject/` segment. Without that field, the entry would say only "the SA called Vertex AI" and the trail back to the Agent would be lost at the SA boundary. With it, the operator can answer "which Entra principal caused this Vertex call?" from a single log entry. This is the field an auditor should index on for cross-cloud attribution.

The `authorizationInfo[0].permission: aiplatform.endpoints.predict` granted on the model resource confirms `roles/aiplatform.user` is in effect for the SA on this project. If this entry shows `granted: false`, the failure is the row-5 case in the §9 failure-modes table: federation and impersonation both worked, but the SA is missing the Vertex role.

### 10.5 Long-term retention with BigQuery

Cloud Audit Log Data Access entries are retained for 30 days in the default `_Default` log bucket. For any timeline that needs more than 30 days of STS / iamcredentials / aiplatform history (a quarterly access review, an audit response, a forensic investigation), the operator sets up a sink that writes the same entries to BigQuery (or Cloud Storage, or a Pub/Sub topic) as they are produced. The sink has to exist before the events occur. It does not retroactively backfill.

A sink covering all three services for this article would be:

```bash
PROJECT=acme-agent-prod-001
DATASET=audit_federation
gcloud auth login
bq --project_id=$PROJECT mk --dataset --location=US $PROJECT:$DATASET

gcloud logging sinks create entra-gcp-federation-sink \
  bigquery.googleapis.com/projects/$PROJECT/datasets/$DATASET \
  --project=$PROJECT \
  --log-filter='protoPayload.serviceName="sts.googleapis.com"
                OR protoPayload.serviceName="iamcredentials.googleapis.com"
                OR protoPayload.serviceName="aiplatform.googleapis.com"' \
  --use-partitioned-tables

# Sinks need writer access to the destination. Read the writer SP from
# the sink and grant it the BigQuery Data Editor role on the dataset:
SINK_WRITER=$(gcloud logging sinks describe entra-gcp-federation-sink \
  --project=$PROJECT --format='value(writerIdentity)')
bq update --source <(echo '{"access":[{"role":"WRITER","userByEmail":"'${SINK_WRITER#serviceAccount:}'"}]}') \
  $PROJECT:$DATASET
```

Once the sink is producing rows, the federation chain for a single chat turn can be reconstructed with one SQL query. The protopayload column in BigQuery is a `RECORD`, so the JSON fields shown in §10.4 become dotted paths:

```sql
-- One row per chat turn, joining the three audit events on a 5-second window.
WITH sts AS (
  SELECT timestamp                                                 AS sts_ts,
         protopayload_auditlog.authenticationInfo.principalSubject AS agent_oid,
         JSON_VALUE(protopayload_auditlog.metadataJson, '$.mapped_principal')
                                                                   AS wif_principal
  FROM `acme-agent-prod-001.audit_federation.cloudaudit_googleapis_com_data_access_*`
...
iamc AS (
  SELECT timestamp                                                 AS iamc_ts,
         protopayload_auditlog.authenticationInfo.principalSubject AS wif_principal,
         protopayload_auditlog.resource.labels.email_id            AS sa_email
  FROM `acme-agent-prod-001.audit_federation.cloudaudit_googleapis_com_data_access_*`
  WHERE protopayload_auditlog.serviceName = 'iamcredentials.googleapis.com'
),
vertex AS (
  SELECT timestamp                                                 AS vertex_ts,
         protopayload_auditlog.authenticationInfo.principalEmail   AS sa_email,
         protopayload_auditlog.authenticationInfo.serviceAccountDelegationInfo[OFFSET(0)].principalSubject
                                                                   AS wif_principal,
         protopayload_auditlog.resourceName                        AS model_resource
  FROM `acme-agent-prod-001.audit_federation.cloudaudit_googleapis_com_data_access_*`
  WHERE protopayload_auditlog.serviceName = 'aiplatform.googleapis.com'
)
SELECT s.agent_oid, s.wif_principal, i.sa_email, v.model_resource,
       s.sts_ts, i.iamc_ts, v.vertex_ts
FROM sts s
JOIN iamc   i ON i.wif_principal = s.wif_principal
              AND TIMESTAMP_DIFF(i.iamc_ts,   s.sts_ts,  SECOND) BETWEEN 0 AND 5
JOIN vertex v ON v.wif_principal = s.wif_principal
              AND TIMESTAMP_DIFF(v.vertex_ts, s.sts_ts,  SECOND) BETWEEN 0 AND 5
ORDER BY s.sts_ts DESC
LIMIT 100;
```

The join key across the three services is the WIF principal URI, which appears verbatim in all three (as `metadata.mapped_principal` in STS, as `authenticationInfo.principalSubject` in iamcredentials, and as `serviceAccountDelegationInfo[0].principalSubject` in aiplatform). This is what makes the cross-service join possible without correlation IDs.

Running the 2-way `iamc ⋈ vertex` reduction of this query against the live deployment for this article, over the same window in which the chat turns were issued (PII-scrubbed for publication, real project `acme-agent-prod-001`, real Agent OID `aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, real WIF pool `entra-prod-pool`), returns four joined rows for two chat turns:

| wif_principal (subject portion)              | sa_email                                                       | model                  | iamc_ts (UTC)        | vertex_ts (UTC)      | gap_ms |
|----------------------------------------------|----------------------------------------------------------------|------------------------|----------------------|----------------------|-------:|
| `.../subject/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee` | `weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com` | `gemini-2.5-flash-lite` | `2026-06-04T06:28:17Z` | `2026-06-04T06:28:18Z` |    197 |
| `.../subject/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee` | `weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com` | `gemini-2.5-flash-lite` | `2026-06-04T06:28:17Z` | `2026-06-04T06:28:19Z` |  1,958 |
| `.../subject/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee` | `weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com` | `gemini-2.5-flash-lite` | `2026-06-04T06:14:55Z` | `2026-06-04T06:15:27Z` | 32,267 |
| `.../subject/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee` | `weather-agentid@acme-agent-prod-001.iam.gserviceaccount.com` | `gemini-2.5-flash-lite` | `2026-06-04T06:14:55Z` | `2026-06-04T06:15:28Z` | 33,123 |

Each chat turn produced one iamc row (the SA access token minted from the federated token) and two Vertex rows (LangChain issues one Gemini call to plan the tool invocation and a second to summarize the weather-API response), so the JOIN expands to four rows. Three findings worth carrying forward to a production runbook:

- **The iamc → vertex chain is the per-call backbone.** Every Vertex row shows a non-NULL `serviceAccountDelegationInfo[0].principalSubject` containing the WIF principal URI, and that URI matches the iamcredentials entry within a minute. The JOIN above keys on that single field and needs nothing else.
- **STS entries are emitted once per WIF token lifetime, not once per chat turn.** The google-auth library caches the federated access token returned by `sts.ExchangeToken` for its full ~1-hour lifetime, so a single STS audit entry covers many downstream iamc/vertex calls. The full three-way `sts ⋈ iamc ⋈ vertex` join returns one row per WIF refresh, not one row per turn. For per-turn forensics, start from `iamc ⋈ vertex` and pull the matching STS row in a second pass.
- **The `_*` wildcard is for the legacy date-sharded layout.** A sink created with `--use-partitioned-tables` (as above) writes a single timestamp-partitioned table named `cloudaudit_googleapis_com_data_access` with no date suffix. The wildcard is harmless but the table can be referenced by its bare name. The `gap_ms` column captures the time between SA token issuance and the first Vertex call: sub-second on a hot run (rows 1 and 2), tens of seconds on a turn that does intermediate tool I/O on Azure (rows 3 and 4). This is a normal tool-loop signature, not a token-issuance latency, so anomaly thresholds on `gap_ms` should be set against the long tail rather than the median.

### 10.6 One chat turn, end-to-end

Putting all four surfaces side by side, a single chat turn against the deployment in this article looks like the timeline below. The clocks are real, taken from the same 05:59 window used throughout this section, scrubbed for identifier values but not for relative timing. Wall-clock skew between Azure and GCP is under one second in practice.

| T+ms | Surface | Event | Principal acting | Resource / audience |
|------|---------|-------|------------------|---------------------|
| +0 ms     | Client → Agent  | HTTP POST `/api/chat` arrives at the agent | (none yet)                | Agent FQDN |
| +4 890    | LAW: sidecar    | MSAL begins `Token Acquisition (ClientCredentialRequest)` | UAMI (via FIC → Blueprint) | Entra v2 token endpoint |
| +7 942    | LAW: sidecar    | MSAL `Token Acquisition finished successfully` (GCP-leg JWT)        | Blueprint (asserting Agent OID)    | `api://<Blueprint>` |
| +8 221    | GCP audit: STS  | `ExchangeToken` accepts the JWT, returns federated access token     | Agent OID (subject in JWT)         | WIF provider audience |
| +8 306    | GCP audit: IAMC | `GenerateAccessToken` issues `ya29.*` for the SA                    | WIF principal URI                  | `weather-agentid@…` |
| +8 915    | LAW: sidecar    | `Token Acquisition finished successfully` (Weather-API-leg JWT)     | Blueprint (asserting Agent OID)    | `api://weather-api-prod` |
| +9 717    | GCP audit: AI   | `GenerateContent` on `gemini-2.5-flash-lite`                        | SA, delegated from WIF principal   | `projects/.../publishers/google/models/gemini-2.5-flash-lite` |
| +9 999    | LAW: weather-api| `[FULL VALIDATION]`, `[AUDIENCE CHECK]`, `[ROLE CHECK]` all pass    | Agent OID (in `aud`/`appid`/`roles`)| `dddddddd-...` |
| +11 519   | LAW: weather-api| `[WEATHER REQUEST] City: London, Temp: 57°F`                        | Agent OID                          | (downstream API logic) |
| +~5 min   | Entra logs      | Two service-principal sign-ins surface (Agent→Blueprint, Agent→API) | Agent SP (`agentType: agenticAppInstance`) | Blueprint and Protected API |

The Entra sign-in row is intentionally last in the table because Entra propagation lags GCP by several minutes. By the time the Entra sign-in is queryable, GCP has had all three of its entries for nearly five minutes. An operator writing a real-time monitor should pull from GCP for the federation chain and use Entra as the slow-but-authoritative source for the identity-side state. An operator writing a daily reconciliation should pull from both and join on the WIF principal URI (GCP side) and the Agent OID (Entra side). The two are the same GUID and the join is exact.
